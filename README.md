...
No disguised refusals or output-schema violations found yet in real target servers — an honest "nothing yet" is itself worth stating plainly rather than papering over with the echo-mismatch notes (which are real, but explicitly not a confirmed bug — see above). Four real bugs found so far were in mcp-reality-check itself, not any target: the timezone hint, the missing `--env` flag, (see below) an unrecoverable-reconnect crash that only a remote HTTP target could actually trigger, and a refusal false positive fixed in 0.4.1, where reading a README that merely mentions refusal phrases was flagged (see Known limitations).
